    ...
    try {
        if (!client) {
            ...
            if (!(await client.accounts()).length) {
                const account = await client.login(WEB3_STORAGE_EMAIL || 'you@example.org');
                const space = await client.createSpace(WEB3_STORAGE_SPACE || 'default-space');
                await space.save();
...
export const uploadJSONToIPFS = async (jsonData) => {
    try {
        ...
        // 개별 파일 업로드
        const cid = await client.uploadFile(blob); // ✅ `uploadDirectory` 대신 `uploadFile` 사용
